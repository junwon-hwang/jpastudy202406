...
class QueryDslSubqueryTest {
    ...
    @Test
    @DisplayName("각 그룹별로 가장 최근에 발매된 앨범의 발매 연도 조회")
    void testFindGroupsWithLatestAlbumReleaseYear() {

        QAlbum subAlbum = new QAlbum("subAlbum");

        // 서브쿼리: 각 그룹별로 가장 최근에 발매된 앨범의 발매 연도
        JPQLQuery<Integer> subQuery = JPAExpressions
                .select(subAlbum.releaseYear.max())
                .from(subAlbum)
                .where(subAlbum.group.id.eq(group.id));

        // 메인쿼리: 그룹 이름과 서브쿼리 결과(가장 최근에 발매된 앨범의 발매 연도) 조회
        List<Tuple> result = factory
                .select(group.groupName, subQuery)
                .from(group)
                .fetch();
        ...
        for (Tuple tuple : result) {
            String groupName = tuple.get(group.groupName);
            Integer latestAlbumReleaseYear = tuple.get(subQuery);

            System.out.println("\nGroup: " + groupName + ", Latest Album Release Year: " + latestAlbumReleaseYear);
        }
    }
